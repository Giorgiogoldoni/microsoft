...
def main():
    ...
    # ── 3LMS.MI (dalla nascita: 11/11/2021) ───────────
    print("Scarico 3LMS.MI...")
    etp = yf.download("3LMS.MI", start="2021-11-11", interval="1d",
                      auto_adjust=True, progress=False)

    # Fallback: yf.download a volte restituisce storico incompleto su ETP
    # a leva poco scambiati. yf.Ticker().history() usa un endpoint diverso
    # e spesso recupera lo storico dove il primo fallisce.
    if len(etp) < 30:
        print(f"⚠️ yf.download ha restituito solo {len(etp)} barre per 3LMS.MI, provo con Ticker().history()...")
        etp = yf.Ticker("3LMS.MI").history(period="max", interval="1d", auto_adjust=True)
        etp.index = etp.index.tz_localize(None) if etp.index.tz is not None else etp.index
        print(f"   → Ticker().history() ha restituito {len(etp)} barre")
    ...
    if len(etp_closes) < 30:
        print("⚠️ ATTENZIONE: storico ancora insufficiente per calcolare KAMA/RSI/SAR/Segnali (servono almeno ~25-30 barre). "
              "Verificare manualmente su finance.yahoo.com se il ticker 3LMS.MI ha storico completo.")
